...
Для оркестрации будет использоваться Kubernetes. Весь поступающий в кластер трафик будет проходить через ingress-nginx-controller.
...
2. отбор объявлений по интересам (самая высокая селективность из всех критериев)
  * если в системе присутсвуют данные об интересах пользователя, используем их
  * если данных о пользователе нет, используем контекстные данные (при наличие): текущую дату / близость к праздникам / регион, из которого пришёл запрос.
  * если контекстных данных нет или их недостаточно, пропускаем этот этап.
3. отсеиваем по городу
4. отсеиваем по остальным критериям
5. проводим аукцион - побеждает объявление с самым высоким значением bid
...
| ----------------- | ----------- | --------- | ------ |
| Подбор объявлений | 68000       | 6800      | 664 ГБ |
...
Запись в ClickHouse будем производить асинхронно раз в секунду. За это время будет накапливаться 204 000 строк. При размере строки в 90 Байт получим 17.5 Мбайт или 140 Мбит.
...
| ClickHouse      | 39 ТБ     | 2032 |  68312  | 683       | 664 ГБ | 1.5 Мбит / c | 140 Мбит / c (раз в 1 с.)
...
| ----------------- | ----- | ----- | ----- | ----- | --- |
| Подбор объявлений | 8     | 8     | 1ГБ   | 1ГБ   | 850 |
