...
В результате трафик попадает на балансировщики. На каждый балансировщик трафик может прийти с двух свитчей, что способствует обеспечению надёжности системы. Балансировщики делятся на 3 группы:
1. Балансировщики фронт-офиса: через них трафик попадает напрямую на сервисы отдачи рекламы
2. Балансировщики бек-офиса: обеспечиают балансировку на остальные сервисы
3. Балансировщики управляющих данных: обеспечивают возможность администрирования даже при высокой загруженности системы
...
Для оркестрации будет использоваться Kubernetes. Для балансировки между сервисами используются те же L7 балансировщики. Также на них происходит терминация ssl. Их конфигурирование происходит через Service Discovery.
...
| last_active | timestamp | 8 Байт |
| last_updated | timestamp | 8 Байт |
...
| Ad history          | 50 Байт             | 164 млрд.    | 7.5 Тб       | 2032  | 204 000 | 
...
Для избежания join'ов по огромным таблицам будем записывать интересы пользователей прямо в visitors, все данные по целевой аудитории напрямую в ads.

Чтобы обеспечить 70 000 RPS будем заранее подбирать объявления батчами по 50 штук для каждого пользователя заранее и сохранять их в отдельной таблице. При 91 млн. пользователей получим 4.5 млрд. строк.
...
| Таблица           | Ключ шардирования | Ключ партиционирования | Размер строки | Кол-во строк | Объём |
| ----------------- | ----------------- | -------- | -------- | - | - |
| ads               | interest_id       | id | 409 Байт | 18 млрд. | 6.7 Тб |
| visitors          | id                | interest | 58 Байт  | 4.5 млрд. | 243 Гб |
| ad recommendations | visitor_id | shown, bid | 392 Байта | 4.5 млрд. | 1.6 ТБ |

### Ad history

- Update'ы в ClickHouse отсутствуют, поэтому на каждый показ / клик / конверсию будет создаваться новая строка. 

### Сводка

| СУБД       | Функционал | Таблицы | Производительность | Надежность | Объём | Нагрузка на чтение(RPS) | Нагрузка на запись(RPS) |
| ---------- | ---------- | ------- | ------------------ | ---------- | ----- | ------------------ | ------------------ |
| ScyllaDB   | Хранение рекламных объявлений, данных о посетителях | ads, visitors, visitor_interests | Горизонтальное масштабирование за счёт создания новых нод (node), шардирование  | Реплицирование данных в рамках одного кластера. По умолчанию replication factor = 3 | 10 ТБ | 70448 | 32 |
| ClickHouse | Хранение истории просмотров для создания отчётов, обновления интересов пользователей | Ad history | Шардирование данных | Горячее резервирование | 13 ТБ | 2032 | 68312 |
...
Для обеспечения 70 000 RPS будем осуществлять фильтрация объявлений заранее.
Для каждого активного в течение месяца пользователя подбираем асинхронно по 50 объявлений и кэшируем результаты подбора в таблицу ad recommendations.

### Фильтрация

1. Отбираем группу давно не обновлявшихся пользователей с похожими интересами.
2. отбираем рекламные интересы по интересам
3. для каждого пользователя отбираем их этих объявлений подходящие по городу / остальным критериям (пол, возраст, ...)

### Подбор

1. Отбираем объявления из таблицы ad recommendations
2. Проводим среди них аукцион - *побеждает ещё не показанное объявление с самым высоким значением bid.

*Если все объявления из выборки уже были показаны, а обновление ещё не успело произойти, показываем те же объявления по 2 кругу.

### Подбор интересов пользователей

Интересы присваиваются пользователям на основе их истории показов и кликов. Если он кликает по объявлению с определёнными интересами, добавляем их ему, если пропускает - удаляем.
...
<table>
    ...
    <tr>
      ...
      <td>
      <ul>
        <li>2 активных энерговвода</li>
        <li>Дублирование всех критических компонентов, разнесение их по разным помещениям</li>
      </ul>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        <ul>
          <li>Репликация данных с replication factor = 2</li>
          <li>Располагаем серверы в двух различных помещениях, соединяем их по средством stretch cluster'а. В случае выхода их строя одной части кластера, вторая часть будет иметь все данные</li>
        </ul>
      </td>
    </tr>
    <tr>
      ...
      <td>
        <ul>
            ...
            <li>отдельный балансировщик для управленческого трафика</li>
        </ul>
      </td>
    </tr>
    <tr>
...
## Нагрузка на обновление рекоммендаций

Допустим интересы равномерно распределены по посетителям. Тогда всех посетителей можно разделить на 90 000 групп. Для гарантированного обновления рекоммендаций всех пользователей не реже 1 раза в 5 минут необходимо добиться RPS не меньше 300.

## Нагрузка на сервисы

| Сервис            | пиковый RPS | CPU cores |RAM     |
| ----------------- | ----------- | --------- | ------ |
| Подбор объявлений | 300         | 30        | 3 ГБ   |
| Отдача объявлений | 68000       | 680       | 664 ГБ |
| Сбор статистики   | 68312       | 683       | 664 ГБ |
| Отчёты            | 2000        | 20        | 20 ГБ  |
| Авторизация       | 32          | 1         | 100 МБ |
| API Gateway (клиенты)   | 2032  | 20        | 20 ГБ  |

### Нагрузка на хранилища и СУБД

Так как все данные дублируются, общий объём диска умножим на 3, объектное - на 2
Запись в ClickHouse производится асинхронно, процесс регулируется самим ClickHouse посредством механихма Asynchrounouse Data Inserts [^12]

| Хранилище       | Объём диска | пиковый RPS на чтение | Пиковый RPS на запись | CPU cores | RAM  | Нагрузка на чтение | Нагрузка на запись |
| --------------- | ----------- | --------------------- | --------------------- | --------- | ---- | ----- | --- |
| ScyllaDB        | 24 ТБ       | 70448 | 32  | 704       | 664 ГБ | 240 Мбит/c | 96 Кбит/c |
| ClickHouse      | 39 ТБ     | 2032 |  68312  | 683       | 664 ГБ | 1.5 Мбит / c | 48 Мбит / c (раз в 1 с.)
| Tarantool       | 24 ГБ       | 2032 | 32         | 1         | 32 ГБ  | 62 Мбит / с | 31 Кбит / c |
| CEPH            | 4250 ТБ     | 68312 | 32   | 683       | 664 ГБ | 266 Гбит / c | 128 Мбит / c | 

### Балансировка

Согласно бенчмарку [^11] оптимальнее всего покупать машины с 24 ядрами. Каждая машина обрабатывает около 10 000 RPS. Всего понадобится 7 машин.

## Конфигурация серверов

| Название                 | Хостинг | Конфигурация | Cores | Cnt | Покупка (1шт.) | Амортизация (5 лет) |
| ------------------------ | ------- | ------------ | ----- | --- | ------- | ------ |
| Отдача объявлений        | own | 2xAMD EPYC 7773X/2x16ГБ 3200MHz DDR4/1xNVMe256Gb | 128 | 54 | €14000 | €234 |
| Подбор объявлений        | own | 1xAMD EPYC 7272/2x8ГБ 3200MHz DDR4/1xNVMe256Gb | 12    | 3  | €3500  | €75  |
...
| ----------------- | ----- | ----- | ----- | ----- | --- |
| Подбор объявлений | 8     | 8     | 0.5ГБ | 0.5ГБ | 4   |
| Отдача объявлений | 8     | 8     | 1ГБ   | 1ГБ   | 850 |
| Сбор статистики   | 8     | 8     | 1ГБ   | 1ГБ   | 850 |
| Отчёты            | 4     | 8     | 400Мб | 1ГБ   | 5   |
| Авторизация       | 1     | 1     | 100Мб | 100Мб | 2   |
...
[^11]: [Бенчмарк nginx](https://www.nginx.com/blog/testing-the-performance-of-nginx-and-nginx-plus-web-servers/)
[^12]: [ClickHouse Asynchrounous Inserts](https://clickhouse.com/blog/asynchronous-data-inserts-in-clickhouse)
